...
<p align="center">
  <img src="https://files.manuscdn.com/user_upload_by_module/session_file/310519663373987164/RGBhpTrATxfqkFWd.png" alt="Kuro Stream Logo" width="200"/>
</p>

<p align="center">
  <img src="https://files.manuscdn.com/user_upload_by_module/session_file/310519663373987164/cSvoKZCoYvDYwzPD.png" alt="Kuro Stream Banner"/>
</p>
...
<p align="center">
  ...
  <br>
  <a href="https://github.com/kurostream-dev/kuro-stream/releases"><img src="https://img.shields.io/github/v/release/kurostream-dev/kuro-stream?style=flat-square&label=latest" alt="Release"></a>
  <a href="https://github.com/kurostream-dev/kuro-stream/stargazers"><img src="https://img.shields.io/github/stars/kurostream-dev/kuro-stream?style=flat-square" alt="Stars"></a>
  <a href="https://github.com/kurostream-dev/kuro-stream/issues"><img src="https://img.shields.io/github/issues/kurostream-dev/kuro-stream?style=flat-square" alt="Issues"></a>
</p>

<p align="center">
  <strong>🌐 Live Demo:</strong> <a href="https://kurostream.app">https://kurostream.app</a><br>
...
Kuro Stream is a free, open-source anime player meticulously engineered for Android TV and Amazon Fire TV devices. It is specifically optimized for low-end hardware, such as the 1GB RAM Fire Stick Lite, ensuring a smooth and responsive user experience. The application functions as a media frontend, aggregating streams via user-installed extensions and converting torrents to HTTP through external proxy servers. Playback is handled by either ExoPlayer or VLC, with a strict policy of zero local P2P processing. Key features include AniList progress synchronization, intelligent skip-intro/outro timestamps, multi-source subtitle aggregation, and robust profile isolation with PIN lock for enhanced privacy. Its TV-first interface is designed for intuitive D-pad navigation, making it ideal for living room entertainment. It is crucial to note that Kuro Stream does not host or distribute any content; all media sources are entirely managed and configured by the user.
...
| Feature | Benefit |
|---------|---------|
| **TV-Optimized UI** | Leverages Jetpack Compose Leanback for full D-pad navigation, high-contrast focus states, and zero UI jank, providing a seamless TV experience. |
| **Dual-Player Engine** | Utilizes ExoPlayer (HLS/DASH/HTTP) as the primary player, with VLC as a fallback for RTSP, exotic codecs, and magnet links, ensuring broad compatibility. |
| **1GB RAM Ready** | Engineered with 8-second buffer caps, SurfaceView rendering, hardware decoder priority, and aggressive memory pooling to perform optimally on low-spec devices. |
| **Zero Local P2P** | All torrents are routed through external proxy servers and converted to HTTP streams, preventing the TV device from directly handling BitTorrent protocols. |

### 🔌 Extensions & Sources
| Feature | Benefit |
|---------|---------|
| **Stremio Addon Support** | Allows installation of any Stremio-compatible addon via manifest URL, expanding content access. |
| **CloudStream Plugin Loader** | Supports bundling or sideloading Kotlin AAR extensions that adhere to the `MainAPI` contract, offering flexible source integration. |
| **Unified Multi-Provider Search** | Queries all active sources in parallel, deduplicates results, and ranks them by quality for efficient content discovery. |
| **Same-Torrent Next Episode** | Automatically increments `file_idx` for continuous binge-watching without the need to re-fetch torrent metadata. |

### 🎯 Anime-Specific Features
| Feature | Benefit |
|---------|---------|
| **AniList Sync** | Features OAuth2 authentication, a dedicated watchlist tab, automatic scrobbling at 80% progress, and an offline queue with retry logic for AniList integration. |
| **Skip Timestamps** | Integrates AniSkip and IntroDB for instant intro/outro/recap skip overlays, enhancing the viewing experience. |
| **Multi-Source Subtitles** | Aggregates subtitles from all active providers, deduplicates by language/hash, and offers a ±5s sync slider for precise timing adjustments. |
| **Trailer Support** | Includes YouTube and Cinemeta integration, along with compatibility for custom trailer addons, to provide rich media previews. |

### 🔐 Privacy & Profiles
| Feature | Benefit |
|---------|---------|
| **Profile Isolation** | Maintains separate addon lists, watch histories, and settings for each profile, ensuring personalized and private viewing. |
| **PIN Lock** | Provides 4-digit PIN protection for individual profiles, ideal for parental controls or securing private content. |
| **Local-First Storage** | All user data is stored locally in encrypted DataStore/Room, with optional AniList cloud synchronization for data portability. |
| **Zero Telemetry** | Ensures no analytics, tracking, or data collection by default, upholding user privacy. |
...
<p align="center">
  <img src="https://kurostream.app/screenshots/home.png" width="300" alt="Home Screen"/>
  <img src="https://kurostream.app/screenshots/detail.png" width="300" alt="Anime Detail"/>
  <img src="https://kurostream.app/screenshots/player.png" width="300" alt="Player Overlay"/>
</p>

> 🖼️ *Screenshots hosted on live demo. Preview the UI directly at [https://kurostream.app](https://kurostream.app)*
...
1.  **Enable Unknown Sources** on your Fire TV:
    ...
    ```
2.  **Download the APK** from [GitHub Releases](https://github.com/kurostream-dev/kuro-stream/releases)
...
git clone https://github.com/kurostream-dev/kuro-stream.git
cd kuro-stream
...
-   🐛 **GitHub Issues**: [Report bugs or request features](https://github.com/kurostream-dev/kuro-stream/issues)
-   🗨️ **GitHub Discussions**: [Share setups, extensions, or ideas](https://github.com/kurostream-dev/kuro-stream/discussions)
...
-   🌐 **Live Demo**: [https://kurostream.app](https://kurostream.app)
...
-   [VLC-Android](https://github.com/videolan/vlc-android) (GPL-2.0) – Fallback playback engine
...
-   💖 **GitHub Sponsors**: [Sponsor @kurostream-dev](https://github.com/sponsors/kurostream-dev)
...
> 🔄 **Migration Notice**: This project is currently hosted at `https://kurostream.app`. We plan to migrate to `https://kurostream.pages.dev` soon. All links will be updated—bookmark the GitHub repo for permanent access.

*Note: Placeholders like `kurostream-dev` and `kurostream.app` have been used for GitHub username and live demo URL. Please replace them with your actual details.*
